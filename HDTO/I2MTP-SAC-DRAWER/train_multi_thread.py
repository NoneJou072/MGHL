...
class IORLModel(ModelBase):
    def __init__(self, env, args):
        ...
        self.model_name = f'{self.agent.agent_name}_{self.args.env_name}_num_{7}_seed_{self.args.seed}'
        ...
        self.env1 = GoalEnvWrapper(DrawerCubeEnv(render_mode=None))
        self.env2 = GoalEnvWrapper(DrawerCubeEnv(render_mode=None))
        self.worker1_event = threading.Event()
        self.worker2_event = threading.Event()
        self.main_event = threading.Event()

    def worker1(self, env, name):
        env = self.env1
        while self.total_steps < self.max_train_steps:
            ...
            while True:
                ...
                obs_, r, terminated, truncated, info = env.step(a)  # 更新环境，返回transition
            ...
            self.worker1_event.set()

            # 等待主线程处理完数据
            self.main_event.wait()

    def worker2(self, env, name):
        env = self.env2
        while self.total_steps < self.max_train_steps:
            # Task2
            env.env.TASK_FLAG = 1
    ...
    def train(self):
        ...
        t1 = threading.Thread(target=self.worker1, args=(self.env, 0,))
        t2 = threading.Thread(target=self.worker2, args=(self.env, 1,))
        t1.start()
        t2.start()
        threads.append(t1)
        threads.append(t2)
        ...
        while self.total_steps < self.max_train_steps:
            # 等待子线程将数据发送到主线程
            self.worker1_event.wait()
            self.worker2_event.wait()
            print(f"main total_steps:{self.total_steps}")
            # if self.total_steps >= self.random_steps:
            #     for _ in range(self.update_freq):
            #         self.agent.update()
            #     self.agent.update_target_net()
            #
            # if self.total_steps >= self.random_steps and self.total_steps % self.args.evaluate_freq == 0:
            #     success_rate_drawer, success_rate_place = self.evaluate_policy()
            #     print(
            #         f"self.total_steps:{self.total_steps} \t  success_rate_drawer:{success_rate_drawer}\t \
            ...
            # 清空事件，以便下一次循环
            self.main_event.set()
            self.worker1_event.clear()
            self.worker2_event.clear()
